...
public class SecurityPropertiesImpl implements SecurityProperties {
    ...
    */
	private final String DEF_ROLES_AND_URL_QUERY = 
	        ...
	        + "    WHERE A.USE_YN ='Y' "
	        + ") "
	        + "SELECT DISTINCT URL, 0 AS AUTHORITY, PAGEID "
			+ "FROM ( "
			+ "SELECT "
            ...
            + "UNION ALL "
            + "SELECT B.URL, A.ROLEID AS AUTHORITY, NULL AS PAGEID "
            + "FROM ( "
            + "SELECT ROLEID "
            + "FROM TB_CMM_ROLE "
            + "UNION ALL "
    ...
    */
	private final String DEF_HTTPS_AND_URL_QUERY = 
	        ...
	        + "    WHERE A.USE_YN ='Y' "
	        + ") "
	        + "SELECT DISTINCT URL, CHANNEL, PAGEID "
	        + "FROM "
	        + "( "
			+ "SELECT URL, "
			...
			+ "       PAGEID "
            + "FROM ( "
            + "SELECT CONCAT(CTE.DMN, C.URL) AS URL, B.HTTPS_USE_YN, B.PAGEID "
            + "FROM TB_CMM_MENU B "
            + "LEFT OUTER JOIN TB_CMM_PRGRM C "
            + "ON B.PRGRMID = C.PRGRMID "
            ...
            + "AND URL != '' "
            + "UNION ALL "
            + "SELECT CONCAT(CTE.DMN, S2.URL) AS URL, S2.CHANNEL, NULL AS PAGEID "
            + "FROM CTE "
            + "CROSS JOIN "
            + "( "
